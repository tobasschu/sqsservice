...
public class DefaultSQSQueue implements SQSQueue {
	...
	public DefaultSQSQueue(final String accessKey, final String secretKey,
			final String queueName) {
		super();
		final AWSCredentials credentials = new BasicAWSCredentials(accessKey,
				secretKey);
		this.sqs = new AmazonSQSClient(credentials);
		this.sqs.setRegion(com.amazonaws.regions.Region
	...
	public synchronized Message receiveMessage() {
		final ReceiveMessageRequest receiveMessageRequest = new ReceiveMessageRequest(
				this.queueUrl).withWaitTimeSeconds(WAIT_TIME_SECONDS)
				.withMaxNumberOfMessages(MAX_NUMBER_OF_MESSAGES)
				.withVisibilityTimeout(VISIBILITY_TIMEOUT);
		final ReceiveMessageResult receiveMessage = this.sqs
	...
	@Override
	public void changeMessageVisibility(final String receiptHandle,
		...
		final ChangeMessageVisibilityRequest changeMessageVisibilityRequest = new ChangeMessageVisibilityRequest(
				this.queueUrl, receiptHandle, retrySeconds);
